...
def test_check_exploration_budget():
    ...
    researcher = DeepResearcher(
        search_function=mock_search_function, exploration_budget=10
    )
...
def test_analyze_diminishing_returns():
    ...
    researcher = DeepResearcher(
        search_function=mock_search_function, diminishing_returns_threshold=0.2
    )
...
def test_integration_with_fast_filter():
    ...
    mock_filter.filter_batch.return_value = [MockFilterResult("Filtered content", 0.9)]

    researcher = DeepResearcher(
        search_function=mock_search_function, fast_filter=mock_filter
    )
    ...
    relevance = researcher._analyze_path_relevance(
        "test query child", "test query", context
    )
...
def test_research_with_budget():
    ...
    researcher = DeepResearcher(
        search_function=mock_search_function, max_depth=2, exploration_budget=5
    )
...
def test_cached_results():
    ...
    researcher = DeepResearcher(
        search_function=counting_search, max_depth=1  # Limit depth to simplify test
    )
    ...
    ), "Results should not be cached between research calls"
