...
from llm_websearch.llm_integration import LLMResponse, ModelTier
...
class TestDeepSearchIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_llm_responses = {
            ...
            "theme_extraction": LLMResponse(
                ...
                processing_time=0.4,
                confidence_score=0.9,
                quality_metrics={"length_score": 0.8, "format_score": 1.0, "elements_score": 0.9},
                timestamp=datetime.now(),
                error=None
            ),
            "relevance_analysis": LLMResponse(
                ...
                processing_time=0.2,
                confidence_score=0.9,
                quality_metrics={"length_score": 0.7, "format_score": 1.0, "elements_score": 1.0},
                timestamp=datetime.now(),
                error=None
            ),
            "summarization": LLMResponse(
                ...
                quality_metrics={"length_score": 1.0, "format_score": 1.0, "elements_score": 0.9},
                timestamp=datetime.now(),
                error=None
            )
        }
    ...
    def mock_llm_response(self, prompt, system_prompt, task_type, required_capabilities=None):
        ...
        return LLMResponse(
            ...
            quality_metrics={"length_score": 0.8, "format_score": 1.0, "elements_score": 0.8},
            timestamp=datetime.now(),
            error=None
        )
    ...
    def test_deep_search_with_iteration(self, mock_llm_integration, mock_search):
        ...
        self.assertGreater(mock_search.call_count, 1, "Search function should be called multiple times")
        
        # Verify that subqueries were generated
        self.assertGreater(
            mock_llm_instance.generate_response.call_count, 1,
        ...
        self.assertGreater(len(result["key_findings"]), 0, "Should have at least one key finding")
        
        # Check that result contains exploration parameters
        self.assertIn("exploration_parameters", result, "Result should include exploration parameters")
        
        # Ensure the output contains tiered summaries
        self.assertIn("tiered_summaries", result, "Result should include tiered summaries")
    
    @patch('llm_websearch.core.search')
    @patch('llm_websearch.llm_integration.LLMIntegration')
    def test_relevance_threshold_controls_exploration(self, mock_llm_integration, mock_search):
        ...
        self.assertGreaterEqual(
            search_count_low, search_count_high,
            "Lower relevance threshold should allow more search paths to be explored"
        )
    
    @patch('llm_websearch.core.search')
    @patch('llm_websearch.llm_integration.LLMIntegration')
    def test_diminishing_returns_threshold_controls_pruning(self, mock_llm_integration, mock_search):
        ...
        def custom_llm_response(prompt, system_prompt, task_type, required_capabilities=None):
            if task_type == "relevance_analysis":
                score = relevance_scores[min(relevance_index[0], len(relevance_scores)-1)]
                relevance_index[0] += 1
                return LLMResponse(
                    content=str(score),
                    model_used="gemini-2.0-flash",
            ...
            return self.mock_llm_response(prompt, system_prompt, task_type, required_capabilities)
        ...
        self.assertGreaterEqual(
            search_count_high_threshold, search_count_low_threshold,
